...
std::unordered_set<int> Hierarchy::bl_index(std::unordered_map<int, std::unordered_map<int, std::unordered_set<std::string>>> &edgeLables, int querynode, int k, Graph &tempgraph, std::unordered_set<std::string> &labels, LCS &lcs, bool isLCS)
{
	if (!isLCS)
	{
		tempgraph.filterEdgeLableAndRecover(labels);
	...
	if (!isLCS)
	{
		...
		temp.filterEdgeLableAndRecover(s2);
		ans = CommunitySearch::onlineWithEdge(temp, k, s);
		// 添加 BFS，获取包含查询节点的连通分量
