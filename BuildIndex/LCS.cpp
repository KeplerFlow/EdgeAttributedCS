...
void LCS::incIndex(std::unordered_map<int, std::unordered_set<int>> &adj,int v,int u,std::unordered_set<std::string> labels,std::string label){
    int root;
    ...
    while (!pq.empty()) {
        auto [vertex, currentCd] = pq.top();
        pq.pop();
        inQueue[vertex] = false;

        // 如果cd[vertex]已经更新且更大，则跳过当前处理，等待重新入队
        if (currentCd != cd[vertex]) {
            continue;
        }
        ...
        if (currentCd > k) {
            ...
            // 否则处理其邻居节点，尝试更新邻居的cd值
            for (int neighbor : adj[vertex]) {
                if (cd[neighbor] > cd[vertex]) {
                    cd[neighbor]--;
                    if (!inQueue[neighbor]) {
                        pq.push({neighbor, cd[neighbor]});
    ...
    for(int vertex:Vstar){
        ...
        {
            if (isSubSet(LcsIndex[vertex][i], labels))
            {
                break; // 一旦发现子集就停止检查
            }
            LcsIndex[vertex][i].push_back(labels);
        }       
    }
    ...
    for (const auto &outer_pair : LcsIndex)
    {
        ...
        for (const auto &inner_pair : outer_pair.second)
        {
            ...
            for (const auto &label_set : inner_pair.second)
            {
                ...
                {
                    total_memory += sizeof(label);    // std::string 对象本身的大小
...
void LCS::serialize(std::ofstream &ofs) const
{
    ...
    // 序列化 subLabels
    size_t subLabels_size = subLabels.size();
    ofs.write(reinterpret_cast<const char *>(&subLabels_size), sizeof(subLabels_size));
    for (const auto &label_set : subLabels)
    {
        size_t set_size = label_set.size();
        ofs.write(reinterpret_cast<const char *>(&set_size), sizeof(set_size));
        for (const auto &label : label_set)
        {
            size_t label_size = label.size();
            ofs.write(reinterpret_cast<const char *>(&label_size), sizeof(label_size));
            ofs.write(label.c_str(), label_size);
        }
    }

    // 序列化 bfsLabels
    size_t bfsLabels_size = bfsLabels.size();
    ofs.write(reinterpret_cast<const char *>(&bfsLabels_size), sizeof(bfsLabels_size));
    for (const auto &pair : bfsLabels)
    {
        ofs.write(reinterpret_cast<const char *>(&pair.first), sizeof(pair.first)); // k值
        size_t vector_size = pair.second.size();
        ofs.write(reinterpret_cast<const char *>(&vector_size), sizeof(vector_size));
        for (const auto &label_set : pair.second)
        {
            size_t set_size = label_set.size();
            ofs.write(reinterpret_cast<const char *>(&set_size), sizeof(set_size));
            for (const auto &label : label_set)
    ...
    // 序列化 bfsCoreness
    size_t bfsCoreness_size = bfsCoreness.size();
    ofs.write(reinterpret_cast<const char *>(&bfsCoreness_size), sizeof(bfsCoreness_size));
    for (const auto &outer_pair : bfsCoreness)
    {
        ofs.write(reinterpret_cast<const char *>(&outer_pair.first), sizeof(outer_pair.first)); // k值
        size_t inner_map_size = outer_pair.second.size();
        ofs.write(reinterpret_cast<const char *>(&inner_map_size), sizeof(inner_map_size));
        for (const auto &inner_pair : outer_pair.second)
        {
            ofs.write(reinterpret_cast<const char *>(&inner_pair.first), sizeof(inner_pair.first));   // coreness键
            ofs.write(reinterpret_cast<const char *>(&inner_pair.second), sizeof(inner_pair.second)); // coreness值
        }
    }
}
...
void LCS::deserialize(std::ifstream &ifs)
{
    ...
    // 反序列化 subLabels
    size_t subLabels_size;
    ifs.read(reinterpret_cast<char *>(&subLabels_size), sizeof(subLabels_size));
    subLabels.clear();
    for (size_t i = 0; i < subLabels_size; ++i)
    {
        size_t set_size;
        ifs.read(reinterpret_cast<char *>(&set_size), sizeof(set_size));
        ...
        subLabels.push_back(label_set);
    }

    // 反序列化 bfsLabels
    size_t bfsLabels_size;
    ifs.read(reinterpret_cast<char *>(&bfsLabels_size), sizeof(bfsLabels_size));
    bfsLabels.clear();
    for (size_t i = 0; i < bfsLabels_size; ++i)
    {
        int k_value;
        ifs.read(reinterpret_cast<char *>(&k_value), sizeof(k_value));
        ...
        bfsLabels[k_value] = label_sets;
    }

    // 反序列化 bfsCoreness
    size_t bfsCoreness_size;
    ifs.read(reinterpret_cast<char *>(&bfsCoreness_size), sizeof(bfsCoreness_size));
    bfsCoreness.clear();
    for (size_t i = 0; i < bfsCoreness_size; ++i)
    {
        int k_value;
        ifs.read(reinterpret_cast<char *>(&k_value), sizeof(k_value));
        ...
        {
            int coreness_key;
            ifs.read(reinterpret_cast<char *>(&coreness_key), sizeof(coreness_key));

            int coreness_value;
            ifs.read(reinterpret_cast<char *>(&coreness_value), sizeof(coreness_value));

            inner_map[coreness_key] = coreness_value;
        }
        bfsCoreness[k_value] = inner_map;
    }
}
