...
int main(int argc, char *argv[])
{
    ...
    Graph graph1("/home/asc23/lcy/graph/data/semanticscholar/mini_sem_network2.txt");

    std::unordered_set<std::string> lables = {"0", "1", "2", "3", "4", "5", "6", "7"};
    std::unordered_set<std::string> queryLabels = {"1", "6"};

    // BFS
    std::vector<int> queryNodes;

    queryNodes.push_back(12956);
    for (int i = 3; i <= 6; i++)
    {
        ...
        for (int j = 0; j < 2; j++)
        {
            ...
            // {
            //     int sum = 0;
            //     for (int n : tempgraph.getNeighbors(temp.front()))
            //     {
            //         if (solution4.find(n) != solution4.end())
            //             sum++;
            //     }
            //     std::cout << "1-hop's neighbors's number is " << sum << std::endl;
            // }
            while (!temp.empty())
            {
                int w = temp.front();
