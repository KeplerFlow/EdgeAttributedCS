...
int main(int argc, char *argv[])
{
    ...
    for (int in = 0; in < 4; in++)
    {
        ...
        for (int j = 0; j < 5; j++)
        {
            ...
            std::unordered_set<int> solution1 = hierarchy.bl_mupt_index(edgeLables, query, k, temp, queryLabels, optLcs, false);
            // if (solution1.size()==1)continue;

            auto query1 = std::chrono::high_resolution_clock::now();

            std::unordered_set<int> solution4 = hierarchy.bl_mupt_index(edgeLables, query, k, graph1, queryLabels, optLcs, true);
            // if (solution4.size()==1)continue;

            auto query4 = std::chrono::high_resolution_clock::now();

            if (solution1.size() == solution4.size())
            {
                std::cout << "k为 " << k << " 时,CL-Tree的平均查询结果为" << solution1.size() << " 查询时间为 " << std::chrono::duration_cast<std::chrono::milliseconds>(query1 - begin).count() << std::endl;
                std::cout << "k为 " << k << " 时,索引结构的平均查询结果为" << solution4.size() << " 查询时间为 " << std::chrono::duration_cast<std::chrono::milliseconds>(query4 - query1).count() << std::endl;
                std::cout << std::endl;
                vaild++;
                indexCost += std::chrono::duration_cast<std::chrono::milliseconds>(query4 - query1).count();
                baseline += std::chrono::duration_cast<std::chrono::milliseconds>(query1 - begin).count();
            }
        }
        std::cout << "加速比为 " << (baseline / vaild) / (indexCost / vaild) << std::endl;
