...
namespace core
{
  ...
  GLIST::GLIST(const GLIST &other)
      : n_(other.n_),
        ...
        tree_(other.tree_), // 假设 Treap 有一个有效的拷贝构造函数
        heap_(other.heap_), // 假设 MinHeap 也有一个有效的拷贝构造函数
        garbage_(other.garbage_)
  {
    // std::cout << "Deep copy successfully!!" << std::endl;
  }

  GLIST::GLIST(const GLISTData &data)
  {
    n_ = data.n;
    head_ = data.head;
    tail_ = data.tail;
    ...
    rank_ = data.rank;
    root_ = data.root;
    evicted_ = data.evicted;
    visited_ = data.visited;
    tree_ = data.tree; // 确保 Treap 有合适的默认构造函数后再赋值
    heap_ = data.heap; // 确保 MinHeap 有合适的默认构造函数后再赋值
    garbage_ = data.garbage;
  }

  GLISTData GLIST::exportGLISTData()
  {
    GLISTData data;
    data.n = this->n_;
    data.head = this->head_;
    data.tail = this->tail_;
    data.node = this->node_;
    data.mcd = this->mcd_;
    data.deg = this->deg_;
    data.rank = this->rank_;
    data.root = this->root_;
    data.evicted = this->evicted_;
    data.visited = this->visited_;
    data.tree = this->tree_; // 确保Treap有合适的拷贝构造函数
    data.heap = this->heap_; // 确保MinHeap有合适的拷贝构造函数
    data.garbage = this->garbage_;
